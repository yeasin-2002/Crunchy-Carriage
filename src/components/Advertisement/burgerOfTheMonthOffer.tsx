...
export const BurgerOfTheMonth = ({ ...rest }: Props) => {
  return (
    <div
      {...rest}
      className={"m-2 grid grid-cols-1 lg:grid-cols-2 sm:relative "}
    >
      <div
        style={{ backgroundImage: `url(${maze.src})` }}
        className={
          "h-full w-full rounded-l-md bg-black bg-cover bg-center bg-no-repeat p-8 sm:py-20  2xl:p-20"
        }
      >
        <p className="pt-20 text-xl font-bold  text-red-600 2xl:text-2xl">
          THE MENU
        </p>
        <h4 className={"pt-20 text-4xl font-bold text-gray-50  2xl:text-5xl"}>
          BURGER OF THE MONTH
        </h4>
        <p
          className={cn(
        ...
        <button className="btn-green mt-6 py-4 2xl:py-6 2xl:px-4 2xl:text-xl 2xl:mt-4">
          Check it now
        </button>
      </div>
      ...
      <Image
        ...
        width={1000}
        height={1000}
        className="hidden h-full w-full rounded-tr-lg lg:block "
        alt="MR BURGER MAN "
        title="MR BURGER MAN"
      />
    </div>
