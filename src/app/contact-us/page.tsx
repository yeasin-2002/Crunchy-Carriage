...
import { barlowCondensed, barlowCondensedLight, barlowCondensedMedium } from "@/fonts";
...
const ContactUs = () => {
  return (
    <div className="bg-snowfall">
      ...
      <div>
        <h1 className={cn("title  mt-10", barlowCondensed.className)}>GET IN TOUCH WITH US</h1>
        <p className="mx-auto mt-5 w-3/4 text-center text-lg font-semibold text-slate-800">
          Get in touch to discuss your employee wellbeing needs today. Please give us a call, drop us an email or fill
          ...
          {aboutUsContacts.map((content, i) => {
            return (
              <div className="*:text-center">
                ...
                <p className={cn("text-2xl font-bold text-slate-800", barlowCondensedMedium.className)}>
                  {content.title}
                </p>
                ...
                <p className={cn("font-bold", content.footerStyle)}>{content.footer}</p>
              </div>
            );
          })}
        </div>
